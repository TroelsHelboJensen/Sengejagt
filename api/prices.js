...
//
// Henter en pris pr. seng ved at hente pris-sidens HTML og udtrække en pris.
// Strategi pr. side:
...
//
// ADVARSEL: Dette er skrøbeligt. Når PriceRunner/Prisjagt ændrer layout,
// kan udtrækket holde op med at virke. Funktionen fejler pænt pr. seng
// (ok: false) frem for at vælte hele kaldet.
...
async function resolveBed(bed) {
  // Direkte produkt-link foretrækkes; ellers søgesiderne.
  const sources = bed.productUrl
    ? [{ source: 'produktside', url: bed.productUrl }]
    : [
        { source: 'PriceRunner', url: bed.links.priceRunner },
        { source: 'Prisjagt', url: bed.links.prisjagt },
      ]
...
export default async function handler(req, res) {
  const results = await Promise.all(beds.map(resolveBed))
